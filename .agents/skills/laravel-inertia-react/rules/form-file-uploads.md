...
export default function EditProfile({ user }: { user: User }) {
    ...
    return (
        <form onSubmit={submit} className="space-y-6">
            <div>
                ...
                <div className="mt-2 flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        />
                        {data.avatar && (
                            <button type="button" onClick={removeFile} className="text-sm text-red-600 hover:text-red-800">
                                Remove new photo
                            </button>
                        )}
                    </div>
            ...
            {progress && (
                <div className="w-full">
                    ...
                    <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200">
                        <div className="h-full bg-indigo-600 transition-all duration-300" style={{ width: `${progress.percentage}%` }} />
                    </div>
                </div>
            )}
            ...
            <button type="submit" disabled={processing} className="rounded-md bg-indigo-600 px-4 py-2 text-white disabled:opacity-50">
            ...
            <div className="grid grid-cols-4 gap-2">
                {data.images.map((file, index) => (
                    <div key={index} className="relative">
                        <img src={URL.createObjectURL(file)} alt={`Preview ${index}`} className="h-24 w-24 object-cover" />
                        <button
                            type="button"
