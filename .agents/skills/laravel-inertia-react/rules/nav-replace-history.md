...
export default function Index({ products, filters, categories }: ProductsIndexProps) {
    ...
    return (
        <div>
            ...
            <select value={filters.category} onChange={(e) => updateFilter('category', e.target.value)} className="rounded-md border-gray-300">
            ...
            <select value={filters.sort} onChange={(e) => updateFilter('sort', e.target.value)} className="rounded-md border-gray-300">
        ...
        <div>
            <div className="flex border-b">
                {['overview', 'details', 'reviews'].map((tab) => (
                    <button key={tab} onClick={() => switchTab(tab)} className={activeTab === tab ? 'border-b-2 border-blue-500' : ''}>
                        {tab}
                    </button>
                ))}
            </div>
