...
const ExpensiveList = React.memo<Props>(
    ...
    (prevProps, nextProps) => {
        return prevProps.items === nextProps.items && prevProps.filter === nextProps.filter && prevProps.onItemClick === nextProps.onItemClick;
    },
);
...
class Mouse extends React.Component<{ children: (pos: MousePosition) => React.ReactNode }, MousePosition> {
    state = { x: 0, y: 0 };
...
class ThemedButton2 extends React.Component {
    render() {
        return <ThemeContext.Consumer>{(theme) => <button className={theme}>{this.props.children}</button>}</ThemeContext.Consumer>;
    }
}
```
