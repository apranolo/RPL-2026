...
Dokumen ini berisi spesifikasi visual, tata letak, antarmuka TypeScript, dan alur interaksi untuk pembuat halaman (_view_) dan komponen di Modul 3 Kelas B (Contract & Finance Workflow).
...
| `Show.tsx`              | `resources/js/pages/Finance/Contract/Show.tsx`   | GILANG JA'FAR P.   | Halaman detail draf surat perjanjian digital (_Digital Agreement_).             |
...
2.  **Aturan Pengaman**:
    - _Constraint_: Validasi `StoreFundingRequest` di backend akan menolak penyimpanan jika akumulasi persentase seluruh termin **tidak genap 100%**.
    - _Monev Lock_: Tombol proses transfer termin lanjutan dinonaktifkan secara otomatis jika status Laporan Kemajuan di modul Monev belum berstatus `"Disetujui"`.
