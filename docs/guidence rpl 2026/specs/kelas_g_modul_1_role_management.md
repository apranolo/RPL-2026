...
Dokumen ini berisi spesifikasi visual, tata letak, antarmuka TypeScript, dan alur interaksi untuk pembuat halaman (*view*) dan komponen di Modul 1 Kelas G.
...
| `ReviewerProfile.tsx` | `resources/js/pages/Profile/ReviewerProfile.tsx` | ADITIYA SUBAKTI | Form profil reviewer, terintegrasi dengan input bidang keilmuan (*SkillTagInput*). |
| `SkillTagInput.tsx` | `resources/js/components/SkillTagInput.tsx` | ADITIYA SUBAKTI | Komponen input interaktif berbasis *chips/tags* yang dapat ditambah dan dihapus (tombol X). |
...
3.  **Tabel Area (Data Grid)**:
    ...
    *   *Kolom Tabel*: `Nama Pengguna`, `Email`, `Peran & Jurnal`, `Status Undangan`, `Aksi`.
    *   *Kolom Peran & Jurnal*: Tampilkan daftar badge menggunakan `<RoleBadge>` untuk setiap peran yang dimiliki. Jika perannya terikat jurnal tertentu, sertakan nama jurnal di bawah badge peran (misal: `"Editor - Jurnal Teknik"`).
    *   *Kolom Status*: Lencana status undangan (`Active` = Hijau, `Invited` = Kuning/Amber, `Declined` = Merah/Rose).
    *   *Kolom Aksi*: Tombol `<Button variant="destructive" size="sm">` dengan label `"Cabut Peran"` yang memicu dibukanya `<RevokeRoleModal>`.
