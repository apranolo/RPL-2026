...
Dokumen ini berisi spesifikasi visual, tata letak, antarmuka TypeScript, dan alur interaksi untuk pembuat halaman (_view_) dan komponen di Modul 4 Kelas G (Peer Review Workflow).
...
| `Invitation.tsx`            | `resources/js/pages/Review/Invitation.tsx`          | M. RIZQI AKBAR H.    | Halaman bagi reviewer untuk menyetujui (_Accept_) atau menolak (_Decline_) penugasan.         |
...
| `RubricScoreInput.tsx`      | `resources/js/components/RubricScoreInput.tsx`      | RIFAI AIHUNAN        | Komponen form rubrik skor interaktif (poin 1–5) dengan kalkulasi rata-rata _real-time_.       |
...
2.  **Tabel Antrean Tugas**:
    ...
    - _Tabel Tugas Aktif_: Menampilkan Judul Naskah (Tanpa Penulis), Jurnal Target, Sisa Hari (due date countdown dengan warna merah jika < 3 hari), dan tombol aksi `"Mulai Review"`.
...
- **Sisi Dokumen**: PDF viewer menampilkan manuskrip yang sudah dibersihkan metadata penulisnya (_anonymized document_).
...
- Di bagian bawah tabel, tampilkan visualisasi skor agregat rata-rata yang terhitung secara dinamis (_real-time_).

### 3. Komponen `<ReviewMatrixTable assignments={...} />`

- Tabel lebar yang membandingkan:
    - _Baris 1_: Rekomendasi Akhir (`Accept` / `Reject` / dll).
    - _Baris 2_: Total Skor Rata-rata.
    - _Baris 3-N_: Catatan ulasan kualitatif untuk penulis dan catatan rahasia untuk editor.
