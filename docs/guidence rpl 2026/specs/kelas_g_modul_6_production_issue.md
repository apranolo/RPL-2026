...
Dokumen ini berisi spesifikasi visual, tata letak, antarmuka TypeScript, dan alur interaksi untuk pembuat halaman (*view*) dan komponen di Modul 6 Kelas G (Issue & Publishing Workflow).
...
2.  **Daftar Urutan Artikel (`<ArticleSequencer>`)**:
    ...
    *   Integrasikan dengan library `@dnd-kit` untuk memungkinkan admin menggeser/menyeret baris artikel ke atas atau bawah secara interaktif (*drag-and-drop*) guna menentukan urutan letak cetak di daftar isi.
...
*   Setiap baris yang diseret wajib menampilkan visual bayangan (*drag shadow*) dan ikon pegangan seret (`GripVertical` dari `lucide-react`) di sebelah kiri judul.
