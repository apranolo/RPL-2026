# 📋 Ringkasan Perubahan Dokumentasi v1.1 (Opsi A - Extended Scope)
...
│
├── Unsur Evaluasi 1 (e.g., "Visi Misi", weight: 20%)
│   ├── Sub Unsur 1.1 (e.g., "Kejelasan Visi")
│   │   ├── Indikator 1.1.1 (pertanyaan spesifik)
│   │   ├── Indikator 1.1.2
│   │   └── Indikator 1.1.3
...
│       └── Indikator 1.2.2
│
├── Unsur Evaluasi 2 (e.g., "Tata Kelola", weight: 30%)
    └── [nested structure...]
```
...
### 1. **jurnal_mu MVP v1.1 - UPDATED.md** ✅

**Perubahan Utama**:

- ✅ Judul diperbarui: "Full Hierarchical Management" emphasis
- ✅ Development Duration: 14 minggu → **17 minggu**
- ✅ Feature 2 diperluas dari "Dynamic Borang Indikator Management" menjadi "Full Hierarchical Borang Indikator Management" dengan 4 sub-features:
...
- ✅ Database schema: Added 3 new tables (accreditation_templates, evaluation_categories, evaluation_sub_categories)
- ✅ Modified evaluation_indicators table: Added sub_category_id, code, display_order, is_required, is_active columns
- ✅ Added drag-and-drop reordering requirements untuk semua 3 levels
- ✅ Added clone template feature requirements
- ✅ Added audit trail requirements untuk compliance
...
### 2. **jurnal_mu roadmap v1.1 - UPDATED.md** ✅

**Perubahan Utama**:

- ✅ Timeline: 14 minggu → **17 minggu** (85 hari kerja)
- ✅ Target Release Date: 23 April 2026 → **14 Mei 2026**
- ✅ Sprint 1 diperpanjang: 4 minggu → **7 minggu** (41% dari total timeline)
- ✅ Sprint 1 dibagi menjadi 2 fase:
    - **Phase 1A** (Weeks 1-2): Assessment Review Implementation
    - **Phase 1B** (Weeks 3-7): Full Hierarchical Borang Management
- ✅ Timeline adjustment untuk Sprint 2-4 (start dates shifted by +3 weeks)
...
- Sprint 2: Week 5-9 → **Week 8-12** (Pembinaan Module)
- Sprint 3: Week 10-12 → **Week 13-15** (Reviewer Management)
- Sprint 4: Week 13-14 → **Week 16-17** (Data Master + Polish)
...
universities (1) ────── (N) journals
scientific_fields (1) ────── (N) journals
users (1) ────── (N) journals
journals (1) ────── (N) journal_assessments
users (1) ────── (N) journal_assessments (reviewer)
journal_assessments (1) ────── (N) assessment_responses
journal_assessments (1) ────── (N) assessment_attachments
...
accreditation_templates (1) ────── (N) evaluation_categories
evaluation_categories (1) ────── (N) evaluation_sub_categories
evaluation_sub_categories (1) ────── (N) evaluation_indicators

[Hierarchy Flow: Template → Unsur (Category) → Sub Unsur → Indikator]
...
journals (1) ────── (N) coaching_requests
users (1) ────── (N) coaching_requests (requester)
coaching_requests (1) ────── (N) coaching_assignments
users (1) ────── (N) coaching_assignments (reviewer)
coaching_assignments (1) ────── (N) coaching_feedback
...
- ✅ **3 new tables added**: accreditation_templates, evaluation_categories, evaluation_sub_categories
- ✅ **1 table modified**: evaluation_indicators (6 new columns)
- ✅ **Backward compatibility preserved**: Old category/sub_category columns retained
- ✅ **Migration strategy**: Gradual transition, no breaking changes for v1.0 data

### **Development Impact**:

- ✅ **Timeline extension**: +3 weeks (21 hari kerja tambahan)
- ✅ **Sprint 1 workload increase**: 4 weeks → 7 weeks (75% increase)
- ✅ **New dependencies**:
...
- ✅ **Value add**: Super Admin can now manage full accreditation hierarchy without developer intervention
- ✅ **Flexibility**: Support multiple accreditation standards (BAN-PT, internal PTM, future standards)
- ✅ **Compliance**: Audit trail provides full traceability for regulatory requirements
- ✅ **UX improvement**: Drag-and-drop reordering significantly better than manual order input
...
- Update original files when ready to replace:
    - `jurnal_mu MVP v1.1.md` → Replace with UPDATED version
    - `jurnal_mu roadmap v1.1.md` → Replace with UPDATED version
    - `ERD Database.md` → Apply manual updates from this summary
