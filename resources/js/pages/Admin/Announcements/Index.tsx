/**
 * AnnouncementsIndex Component
 *
 * @description
 * A list view for Super Admin to manage announcements across all universities.
 * It supports search, university filtering, active status, published status, and pagination.
 *
 * @features
 * - Search announcements by title, content, or description
 * - Filter by university
 * - Filter by active status and published state
 * - Paginated announcement list with desktop and mobile views
 * - Quick access to create, view, and edit announcement details
 *
...
export default function AnnouncementsIndex({ announcements, universities, filters }: Props) {
    ...
    const hasActiveFilters = Boolean(search || universityId || isActiveFilter || isPublishedFilter);
                ...
                <div className="relative overflow-hidden rounded-xl border border-sidebar-border/70 bg-white p-6 dark:border-sidebar-border dark:bg-neutral-950">
                    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-muted-foreground">Monitor and manage announcements across all universities.</p>
                        </div>
                                        ...
                                        <TableRow key={announcement.id}>
                                            ...
                                            <TableCell>
                                                {announcement.university ? (
                                                    <div>{announcement.university.short_name || announcement.university.name}</div>
                                                ) : (
                                                    <div className="text-muted-foreground">No university</div>
                                                )}
                                            </TableCell>
                                            <TableCell>
                                ...
                                <Card key={announcement.id} className="overflow-hidden border-sidebar-border/70 dark:border-sidebar-border">
                                    <CardContent className="space-y-4 p-4">
                                        ...
                                        <div className="grid gap-2 text-sm text-muted-foreground">
                                            <div>University: {announcement.university?.short_name || announcement.university?.name || 'No university'}</div>
