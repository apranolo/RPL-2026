/**
 * Admin/Proposals/Index — Super Admin
 *
 * @description
 ...
 * - Tabel dengan kolom: Judul, Pengusul, Skema, Status, Tanggal, Aksi
...
function RejectModal({ proposal, onClose }: RejectModalProps) {
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="Tuliskan alasan penolakan secara jelas dan lengkap (min. 10 karakter)..."
                            className={`w-full resize-none rounded-lg border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring ${
                                errors.rejection_reason ? 'border-destructive focus:ring-destructive/30' : 'border-border'
                            }`}
                        />
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                            {data.rejection_reason.length}/500 karakter (min. 10)
                        </p>
                    </div>
...
export default function AdminProposalsIndex({ proposals, filters, statusOptions }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {flashMessage && (
                <div
                    className={`fixed right-4 top-4 z-50 flex items-center gap-2 rounded-lg border px-4 py-3 text-sm shadow-lg transition-all ${
            ...
            {/* ── Reject Modal ── */}
            {rejectTarget && (
                <RejectModal proposal={rejectTarget} onClose={() => setRejectTarget(null)} />
            )}
                                                    ...
                                                    {proposal.status_proposal === 'Submitted' ? (
                                                        <>
                                                            <Button
                                                                ...
                                                                title="Validasi administrasi proposal"
                                                            >
                ...
                {proposals.last_page > 1 && (
                    <div className="flex items-center justify-between">
                        <p className="text-sm text-muted-foreground">
                            ...
                            {Math.min(proposals.current_page * proposals.per_page, proposals.total)} dari{' '}
                            {proposals.total} proposal
                        </p>
                            ...
                            {proposals.links.map((link, idx) => {
                                ...
                                return (
                                    <button
                                        ...
                                        aria-label={isFirst ? 'Halaman sebelumnya' : isLast ? 'Halaman berikutnya' : `Halaman ${link.label}`}
                                    >
                                        {isFirst ? (
