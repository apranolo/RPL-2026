...
import { ArrowLeft, Building2, Calendar, CheckCircle2, CircleX, ClipboardList, FileText, Hash, User, Wallet, Zap } from 'lucide-react';
...
function StatusUpdateForm({ contract }: StatusUpdateFormProps) {
    ...
    const allowedTransitions: Record<
        string,
        { value: string; label: string; icon: React.ElementType; variant: 'default' | 'destructive' | 'outline' }[]
    > = {
        draft: [{ value: 'aktif', label: 'Aktifkan Kontrak', icon: Zap, variant: 'default' }],
        aktif: [
                ...
                <div className="flex flex-wrap gap-3">
                    {transitions.map(({ value, label, icon: Icon, variant }) => (
                        <form key={value} onSubmit={handleSubmit(value)}>
                            <Button id={`btn-status-${value}`} type="submit" variant={variant} disabled={processing}>
                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            <h1 className="text-2xl font-bold tracking-tight">Detail Kontrak Penelitian</h1>
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="space-y-6 lg:col-span-2">
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-5">
                                <div>
                                    <p className="text-xs font-medium tracking-wider text-muted-foreground uppercase">Judul Kontrak</p>
                                ...
                                {contract.description && (
                                    <>
                                        <Separator />
                                        <div>
                                            <p className="text-xs font-medium tracking-wider text-muted-foreground uppercase">Deskripsi</p>
                                            <p className="mt-1 text-sm whitespace-pre-wrap text-muted-foreground">{contract.description}</p>
                                        </div>
                                    </>
                                )}
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    ...
                                    />
                                    <InfoRow icon={Wallet} label="Nilai Kontrak" value={formatCurrency(contract.value)} />
                                    <InfoRow icon={Calendar} label="Tanggal Mulai" value={formatDate(contract.start_date)} />
                                    <InfoRow icon={Calendar} label="Tanggal Berakhir" value={formatDate(contract.end_date)} />
                                </div>
                                ...
                                {contract.notes && (
                                    <>
                                        <Separator />
                                        <div>
                                            <p className="mb-1 text-xs font-medium tracking-wider text-muted-foreground uppercase">Catatan</p>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div>
                                    <p className="text-xs font-medium tracking-wider text-muted-foreground uppercase">Nama Jurnal</p>
                                    <p className="mt-1 text-base font-semibold">{contract.journal.title}</p>
                                </div>

                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    <InfoRow icon={Hash} label="ISSN" value={contract.journal.issn || '—'} />
                                    {contract.journal.e_issn && <InfoRow icon={Hash} label="e-ISSN" value={contract.journal.e_issn} />}
                                    {contract.journal.university && (
                                        <InfoRow icon={Building2} label="Universitas" value={contract.journal.university.name} />
                                    )}
                    ...
                    <div className="space-y-6">
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-3">
                                <InfoRow icon={User} label="Pengelola Jurnal" value={contract.user.name} />
                                {contract.generator && (
                                    <>
                                        <Separator />
                                        <InfoRow icon={User} label="Digenerate Oleh" value={contract.generator.name} />
                                    </>
                                )}
                            </CardContent>
                        </Card>
