...
export default function ReviewerIndex({ tasks, assignments, progressReports, selectedReview }: Props) {
    ...
    const getProposalDescription = (proposal?: Proposal | null) =>
        proposal?.description ?? proposal?.deskripsi ?? 'Proposal belum memiliki deskripsi tambahan.';
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-600">Review proposal yang ditugaskan kepada Anda.</p>
                    </div>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-200 bg-white">
                        {taskData.data.map((task) => (
                            <tr key={task.id}>
                                ...
                                <td className="px-4 py-4 text-sm text-slate-700">{task.assessment_criteria?.length ?? 0} kriteria</td>
                                <td className="px-4 py-4 text-sm text-slate-700">{new Date(task.created_at).toLocaleDateString('id-ID')}</td>
            ...
            >
                <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>Periksa dokumen proposal dan isi formulir evaluasi reviewer.</DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-6">
                            <div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                                    ...
                                    <p className="mt-2 text-sm text-slate-700">{getProposalDescription(activeReview.proposal)}</p>
